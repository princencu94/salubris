                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {trainers.map((trainer) => (
                    <tr key={trainer.id}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{trainer.email}</td>
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <Link href={`/edittrainer/${trainer.id}`} className="text-orange-600 hover:text-orange-900">
                          Edit<span className="sr-only">, {trainer.id}</span>
                        </Link>
                      </td>
                    </tr>
                  ))}
